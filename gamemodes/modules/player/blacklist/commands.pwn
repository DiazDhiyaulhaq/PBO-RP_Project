...
CMD:baninfo(playerid, params[])
{
    ...
    // FIX: Mengubah 'time' menjadi 'timestamp' agar tidak warning shadowing
    new reason[64], banby[24], timestamp, date;

    if(cache_num_rows())
    {
        cache_get_value_int(0, "Temp", timestamp); // Variable 'time' -> 'timestamp'
        ...
        if(timestamp)
            // Fix ConvertTimestamp(Time:...) -> ConvertTimestamp(Time:timestamp)
            Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, "Banned Info", ""WHITE"Ban Lookup: "RED"%s\n"WHITE"Date Banned: "RED"%s\n"WHITE"By Admin/Helper: "RED"%s\n"WHITE"Reason: "RED"%s\n\n"WHITE"Unban Date: "RED"%s", "Close", "", params, ConvertTimestamp(Time:date), banby, reason, ConvertTimestamp(Time:timestamp));
        else
