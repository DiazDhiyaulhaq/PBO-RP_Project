Blacklist_Check(playerid, const type[], target[])
{
    ...
    // FIX: Mengganti 'time' menjadi 'timestamp' agar tidak bentrok dengan fungsi time() global
    new date, timestamp;
    new reason[128], ip[16], username[MAX_PLAYER_NAME], characters[MAX_PLAYER_NAME], banby[MAX_PLAYER_NAME];

    if(cache_num_rows())
    {
        cache_get_value_int(0, "Temp", timestamp); // Update variable
        ...
        {
            // Update variable 'time' -> 'timestamp'
            if(!timestamp)
            ...
            // Update variable 'time' -> 'timestamp'
            else if((timestamp) && (gettime() < timestamp))
                Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, "Temp Ban - Character Banned", ""WHITE"You are banned from this server.\n\nCharacter: "RED"%s\n"WHITE"Date: "RED"%s\n"WHITE"Banned by admin: "RED"%s\n"WHITE"Reason: "RED"%s\n"WHITE"Unban Date: "RED"%s\n\n"WHITE"To request a ban appeal, please visit our website and submit a ban appeal.", "Close", "", ReturnName(playerid), ConvertTimestamp(Time:date), banby, reason, ConvertTimestamp(Time:timestamp));
