...
Dialog:RentVehicle(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        // FIX: Mengganti 'time' menjadi 'rentDuration'
        static
            rentDuration,
            selectedItem,
            prices;
        ...
        rentDuration = GetPVarInt(playerid, "Rent_Time");
        prices = (RentPoint[GetPVarInt(playerid, "holdingRentID")][rentPrice][selectedItem]*rentDuration);
        ...
        VehicleData[id][cRentTime] = (3600*rentDuration);
        ...
        GiveMoney(playerid, -(prices*rentDuration));

        Vehicle_Save(id);

        GetVehicleNameByVehicle(VehicleData[id][cVehicle], vehName);
        DeletePVar(playerid, "Rent_Time");
        SendServerMessage(playerid,"Anda telah menyewa kendaraan "CYAN"%s"WHITE" dengan jangka waktu "YELLOW"%d jam.", vehName, rentDuration);
