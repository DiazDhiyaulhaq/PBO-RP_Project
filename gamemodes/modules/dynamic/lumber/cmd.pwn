...
CMD:lumbermenu(playerid, params[])
{
    ...
    switch(action)
    {
		...
		case 4: // Time
        {
            new interval; // <--- SUDAH DIGANTI

            if(sscanf(nextParams, "dd", lumber_id, interval)) // Ganti time -> interval
                return SendSyntaxMessage(playerid, "/lumbermenu delete <lumber id> <time>");

            if(!Lumber_Exists(lumber_id))
                return SendErrorMessage(playerid, "Id lumber tidak terdaftar diserver.");

            if(interval < 0) // Ganti time -> interval
                return SendErrorMessage(playerid, "Masukkan angka lebih dari nol!.");

            Lumber_SetTime(lumber_id, interval); // Ganti time -> interval
            Lumber_Sync(lumber_id);
            Lumber_Save(lumber_id, SAVE_LUMBER_TIME);

            SendServerMessage(playerid, "Waktu spawn lumber id "YELLOW"%d"WHITE" telah diperbaharui menjadi "GREEN"%d detik!", lumber_id, interval); // Ganti time -> interval
        }
