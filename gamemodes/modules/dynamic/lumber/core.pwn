    ...
    if(Lumber_Exists(lumber_id)) {
        if(type == 1) {
            new cp = LumberData[lumber_id][lumberCP],
            	remainingTime = LumberData[lumber_id][lumberTime];
            ...
            Streamer_SetFloatData(STREAMER_TYPE_CP, cp, E_STREAMER_Z, (remainingTime) ? LumberData[lumber_id][lumberPos][2] : (LumberData[lumber_id][lumberPos][2] - 2));
        }
        else if(type == 2) {
...
Lumber_SetTime(lumber_id, seconds)
{
    if(Lumber_Exists(lumber_id)) {
        LumberData[lumber_id][lumberTime] = seconds;
        return 1;
    }
    return 0;
}
