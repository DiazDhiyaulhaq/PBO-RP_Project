...
enum vipData {
  ...
  vipCode,
  vipType,
  vipTime, // Ini aman karena di dalam enum
  vipGold,
  vipClaim
};
...
// FIX: Mengganti argumen 'time' menjadi 'days' agar tidak warning shadowing
Vip_Create(playerid, code, type, days, gold) {
  for (new id; id < MAX_VIPS; id ++) if (!Iter_Contains(Vips, id)) {
    new query[256]; // Diperbesar sedikit biar aman

    if (id == -1) return SendErrorMessage(playerid, "Cannot create more giftcodes!");

    VipData[id][vipCode] = code;
    VipData[id][vipType] = type;
    VipData[id][vipTime] = days; // Assign days ke enum
...
GetVipTime(playerid) {
  // FIX: Mengganti 'time' menjadi 'timeStr'
  static timeStr[64];

  if (PlayerData[playerid][pVip] >= 1) {
    if (PlayerData[playerid][pVipTime] != 0) {
      // Perbaikan Tag Mismatch: ConvertTimestamp(Time:...)
      format(timeStr,sizeof(timeStr), ""LIGHT_BLUE"%s", ConvertTimestamp(Time:PlayerData[playerid][pVipTime]));
    } else {
      format(timeStr,sizeof(timeStr), ""RED"Expired");
    }
  } else format(timeStr,sizeof(timeStr),""RED"None");
...
CMD:creategiftcode(playerid, params[]) {
  ...
  new
    id,
    type,
    days, // FIX: Mengganti 'time' menjadi 'days'
    code,
    gold
  ;

  if (sscanf(params, "ddd", type, days, gold)) {
  ...
  // Update pemanggilan fungsi dengan variabel baru
  id = Vip_Create(playerid, code, type, days, gold);
...
CMD:redeem(playerid, params[]) {
  ...
  foreach (new i : Vips) {
    if (VipData[i][vipCode] == code) {
      ...
      // Menggunakan gettime() (fungsi global) dengan benar
